...
basisSet2 <- function (modelList.with.data, direction = NULL, interactions = FALSE) {
  amat <- piecewiseSEM:::getDAG(modelList.with.data)
...
multigroup2 <- function(
  ...
  intModelList <- lapply(modelList, function(i) {
    rhs2 <-
      paste(paste(piecewiseSEM:::all_vars_trans(i)[-1], "*", group),
            collapse = " + "
      )
  ...
  newModelList <- lapply(unique(data[, group]), function(i) {
    update(piecewiseSEM:::as.psem(modelList),
  ...
  coefTable <- piecewiseSEM:::coefs(
    modelList, standardize, standardize.type,
    test.type
  )
  #  anovaTable <- anova(piecewiseSEM:::as.psem(intModelList))[[1]]
  anovaTable <- anova(piecewiseSEM:::as.psem(intModelList))[[1]]
      ...
      for (j in 1:nrow(ct)) {
        if (ct[j, ncol(ct)] == "c") {
          model <- modelList[[which(sapply(
            ...
            function(x) {
              piecewiseSEM:::all_vars_merMod(x)[1] == ct[j,"Response"]
            }
          ))]]
          data. <- data[data[, group] == i, ]
          sd.x <-
  ...
  if (nrow(global) == nrow(anovaInts)) {
    ...
    gof <- piecewiseSEM:::fisherC(modelList, basis.set = b)
  }
